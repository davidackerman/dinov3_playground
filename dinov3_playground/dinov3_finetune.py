...
# %%-tuning Pipeline - Main Execution File
...
from dinov3_playground import dinov3_core, data_processing, models, model_training, visualization, data_processing, memory_efficient_training
...
from dinov3_playground.model_training import (
    balance_classes,
...
if USE_FLEXIBLE_SAMPLING:
    ...
    train_images, train_gt = sample_training_data(
        raw, gt, 
        target_size=896,
        num_samples=10, 
        method="simple",
...
plt.subplot(1, 3, 1)
plt.imshow(raw_select[1,], cmap='gray')
plt.title('Raw Image')
plt.subplot(1, 3, 2)
plt.imshow(gt_select[1])
...
memory_results = train_with_memory_efficient_loader(
    ...
    num_classes=2,
    epochs=50,
    target_size=224,       # Final resolution 224x224
    balance_method="weighted",
    model_id=MODEL_ID      # Add model ID
)

print("Memory-efficient training completed!")
# %%
# from dinov3_playground.memory_efficient_training import load_model_and_run_inference
# raw_data = ImageDataInterface("/nrs/cellmap/data/jrc_22ak351-leaf-3m/jrc_22ak351-leaf-3m.zarr/recon-1/em/fibsem-uint8/s2").to_ndarray_ts(
#     Roi(np.array((670, 922, 4086))*32, 3*[750*32])
# )
# load_model_and_run_inference("/groups/cellmap/cellmap/ackermand/Programming/cellmap-flow/dinov3_playground/tmp/results/memory_efficient_training_20250912_153301_latest.pkl", raw_data, gt_data=None, 
#                                  num_sample_images=5, target_size=896, 
#                                  device=None, seed=42, show_results=True)
# %%
...
checkpoints = list_checkpoints("/groups/cellmap/cellmap/ackermand/Programming/cellmap-flow/dinov3_playground/tmp/results/")
