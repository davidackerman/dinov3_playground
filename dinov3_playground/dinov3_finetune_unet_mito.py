...
from dinov3_playground.models import DINOv3UNet, create_model, print_model_summary
...
EXPORT_BASE_DIR = "/nrs/cellmap/ackermand/dinov3_training/results/mito"
...
if "trained_unet" in locals() and "data_loader" in locals():
    ...
    test_datasets = [
        {
            "name": "Hela-2",
            "path": "/nrs/cellmap/data/jrc_hela-2/jrc_hela-2.zarr/recon-1/em/fibsem-uint8/s2",
            "roi": Roi(
                np.array((490, 30, 500)) * base_resolution,
                [500 * base_resolution, 100 * base_resolution, 500 * base_resolution],
            ),
        },
        {
            "name": "Liver-Zon-2",
            "path": "/nrs/cellmap/data/jrc_mus-liver-zon-2/jrc_mus-liver-zon-2.zarr/recon-1/em/fibsem-uint8/s1",
            "roi": Roi(
                np.array((272, 3981, 2290)) * base_resolution,
                [750 * base_resolution] * 3,
            ),
        },
    ]
    ...
    for dataset_idx, dataset_info in enumerate(test_datasets):
        ...
        try:
            ...
            test_features_new = data_loader.extract_dinov3_features_for_unet(
                test_patches
            )
            ...
            with torch.no_grad():
                ...
                new_confidence = (
                    torch.max(torch.softmax(new_pred_logits, dim=1), dim=1)[0]
                    .cpu()
                    .numpy()
                )
            ...
            if checkpoint_dir:
                figure_path = os.path.join(
                    checkpoint_dir,
                    f"inference_{dataset_name.lower().replace('-', '_')}.png",
                )
    ...
    if len(all_results) > 1:
        ...
        for dataset_idx, dataset_name in enumerate(dataset_names):
            ...
            for i in range(6):
                ...
                if i == 0:
                    axes[row_pred, i].set_ylabel(
                        f"Predictions", fontsize=12, rotation=0, ha="right", va="center"
                    )
        ...
        if checkpoint_dir:
            comparison_path = os.path.join(
                checkpoint_dir, "inference_comparison_all_datasets.png"
            )
        ...
        print(
            f"{'Dataset':<15} {'Class 0 %':<10} {'Class 1 %':<10} {'Mean Conf':<12} {'Std Conf':<10}"
        )
        print("-" * 60)
        ...
        for dataset_name, results in all_results.items():
            ...
            print(
                f"{dataset_name:<15} {class0_pct:<10.1f} {class1_pct:<10.1f} {confidence.mean():<12.4f} {confidence.std():<10.4f}"
            )

    # Save comprehensive statistics to text file
    if checkpoint_dir and all_results:
        stats_path = os.path.join(
            checkpoint_dir, "inference_statistics_all_datasets.txt"
        )
        with open(stats_path, "w") as f:
            ...
            for dataset_name, results in all_results.items():
                ...
                for class_id, count in zip(unique_preds, pred_counts):
                    ...
                    f.write(
                        f"  Class {class_id}: {count:,} pixels ({percentage:.1f}%)\n"
                    )
