...
from dinov3_playground.models import DINOv3UNet, create_model, print_unet_summary
...
# Print DINOv3 model summary
print("\n" + "=" * 60)
print("DINOV3 MODEL SUMMARY")
print("=" * 60)
try:
    # DINOv3 expects input shape (batch_size, channels, height, width)
    summary(model, input_size=(3, IMAGE_SIZE, IMAGE_SIZE))
except Exception as e:
    print(f"Could not generate DINOv3 summary: {e}")
    print(f"DINOv3 model type: {type(model)}")
    print(f"DINOv3 parameters: {sum(p.numel() for p in model.parameters()):,}")
print("=" * 60)

print("Loading datasets...")
EXPORT_BASE_DIR = (
    "/nrs/cellmap/ackermand/dinov3_training/results/"  # Updated export directory
)
...
if "unet_results" in locals() and "data_loader" in locals():
    ...
    # Print UNet model summary for the trained model from previous examples
    print("\n" + "=" * 60)
    print("UNET MODEL SUMMARY (From Previous Examples)")
    print("=" * 60)
    try:
        # Get input channels from the model's first layer
        first_conv = None
        for module in trained_unet.modules():
        ...
        if first_conv is not None:
            ...
            summary(trained_unet, input_size=(input_channels, 224, 224))
        else:
            print("Could not determine input channels from model")
            print(f"UNet model type: {type(trained_unet)}")
            print(
                f"UNet parameters: {sum(p.numel() for p in trained_unet.parameters()):,}"
            )
    except Exception as e:
        print(f"Could not generate UNet summary: {e}")
        print(f"UNet model type: {type(trained_unet)}")
        print(f"UNet parameters: {sum(p.numel() for p in trained_unet.parameters()):,}")
    print("=" * 60)
else:
    ...
    if not checkpoints:
        ...
        print("=" * 60)
        try:
            # UNet expects input shape (batch_size, input_channels, height, width)
            # Use the actual input channels from the loaded model
            summary(trained_unet, input_size=(input_channels, 224, 224))
        except Exception as e:
            print(f"Could not generate UNet summary: {e}")
            print(f"UNet model type: {type(trained_unet)}")
        ...
        if config.get("model_id") != current_model_id:
            print(
                f"WARNING: Checkpoint was trained with {config.get('model_id', 'Unknown')}"
            )
            print(f"         but current model is {current_model_id}")
            print(
                f"         This may cause issues if the models have different output dimensions!"
            )
        ...
        data_loader = MemoryEfficientDataLoader(
            raw_data=(
                raw if "raw" in locals() else np.zeros((1, 224, 224), dtype=np.uint8)
            ),
...
if "trained_unet" in locals() and "data_loader" in locals():
    ...
    try:
        raw_test = ImageDataInterface(
            "/nrs/cellmap/data/jrc_22ak351-leaf-3m/jrc_22ak351-leaf-3m.zarr/recon-1/em/fibsem-uint8/s4"
        ).to_ndarray_ts(
            Roi(np.array((0, 0, 0)) * 128, [256 * 128, 256 * 128, 1000 * 128])
        )
        ...
        with torch.no_grad():
            ...
            new_confidence = (
                torch.max(torch.softmax(new_pred_logits, dim=1), dim=1)[0].cpu().numpy()
            )
