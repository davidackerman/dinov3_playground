...
base_resolution = 32  # Resolution for segmentation labels
min_resolution_raw = 8  # Don't use scales finer than 8nm for raw data
...
DINOV3_SLICE_SIZE = 512  # Native resolution of raw data slices
...
print(f"  Raw data resolution: {min_resolution_raw}nm (actual data)")
...
EXPORT_BASE_DIR = "/nrs/cellmap/ackermand/dinov3_training/results/multiple_3d_orthogonal_highest_raw_res_32nm"
...
# Use highest resolution for raw data (for better DINOv3 input) and base_resolution for labels
# Set minimum resolution to prevent using extremely high resolution scales (e.g., 2nm)
# that might be computationally prohibitive without significant benefit

dataset_pairs = update_datapaths_with_target_scales(
    dataset_pairs,
    base_resolution,
    use_highest_res_for_raw=True,  # Use highest available resolution for raw data
    min_resolution_for_raw=min_resolution_raw,  # But not finer than 8nm
)
...
for vol_idx in range(5):  # 5 volumes (columns)
    if vol_idx < raw.shape[0]:
        ...
        for slice_idx, (z, perc_label) in enumerate(
            ...
            raw_slice = raw[
                vol_idx, z * base_resolution // min_resolution_raw
            ]  # Shape: (64, 64)
