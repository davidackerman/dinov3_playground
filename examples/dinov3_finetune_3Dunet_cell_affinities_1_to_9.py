...
min_resolution_for_raw = 32  # Don't use scales finer than 32nm for raw data
...
# Updated to show multi-scale affinities (1, 3, 9 voxel offsets)
fig, axes = plt.subplots(
    30, 10, figsize=(20, 72)
)  # 30 rows (6 types × 5 slices), 10 columns (volumes)
...
# Define affinity offset groups (for multi-scale visualization)
affinity_groups = [
    {
    ...
    {
        "offsets": [(3, 0, 0), (0, 3, 0), (0, 0, 3)],
        "name": "3-voxel",
        "color": "Greens",
    },
    {
        "offsets": [(9, 0, 0), (0, 9, 0), (0, 0, 9)],
        "name": "9-voxel",
        "color": "Blues",
    },
]
...
for vol_idx in range(10):  # 10 volumes (columns)
    if vol_idx < raw.shape[0]:
        ...
        for slice_idx, (z, perc_label) in enumerate(
            ...
            gt_slice = gt[vol_idx, z]  # Shape: (H, W)
            mask_slice = gt_masks[vol_idx, z]  # Shape: (H, W)

            # Process slice through DINOv3
            slice_features = process(raw_slice[np.newaxis, ...])  # Add batch dim

            # Calculate row indices for the 6 image types
            raw_row = slice_idx * 6  # 0, 6, 12, 18, 24
            aff1_row = slice_idx * 6 + 1  # 1, 7, 13, 19, 25
            aff3_row = slice_idx * 6 + 2  # 2, 8, 14, 20, 26
            aff9_row = slice_idx * 6 + 3  # 3, 9, 15, 21, 27
            mask_row = slice_idx * 6 + 4  # 4, 10, 16, 22, 28
            feat_row = slice_idx * 6 + 5  # 5, 11, 17, 23, 29

            # 1. Raw slice with GT overlay
            ...
            if slice_idx == 0:  # Only show volume info on first slice
                axes[raw_row, vol_idx].set_title(
                    f"Vol {vol_idx+1} ({source_name})\nRaw + Instances Z={z} ({perc_label})",
                    fontsize=8,
                )
            else:
                axes[raw_row, vol_idx].set_title(
                    f"Raw + Instances Z={z} ({perc_label})", fontsize=8
                )
            axes[raw_row, vol_idx].axis("off")

            # 2-4. Affinities at different scales (as RGB overlays on raw)
            # Get 3D neighborhood around this slice for affinity computation
            for aff_idx, (aff_group, aff_row) in enumerate(
                zip(affinity_groups, [aff1_row, aff3_row, aff9_row])
            ):
                max_offset = max(
                    [abs(o) for offset in aff_group["offsets"] for o in offset]
                )
                z_start = max(0, z - max_offset)
                z_end = min(gt.shape[1], z + max_offset + 1)
                gt_3d_patch = gt[vol_idx, z_start:z_end]  # Shape: (small_D, H, W)

                # Compute affinities for this scale
                affinities_3d = compute_affinities_3d(
                    gt_3d_patch, offsets=aff_group["offsets"]
                )  # Shape: (3, small_D, H, W)

                # Extract the slice corresponding to z
                slice_idx_in_patch = z - z_start
                if slice_idx_in_patch >= affinities_3d.shape[1]:
                    slice_idx_in_patch = affinities_3d.shape[1] - 1
                affinity_slice = affinities_3d[
                    :, slice_idx_in_patch, :, :
                ]  # Shape: (3, H, W)

                # Stack as RGB image (transpose to H, W, 3)
                affinity_rgb = np.transpose(
                    affinity_slice, (1, 2, 0)
                )  # Shape: (H, W, 3)

                # Plot raw with affinity overlay
                axes[aff_row, vol_idx].imshow(raw_slice, cmap="gray")
                # Overlay affinities as RGB with transparency
                axes[aff_row, vol_idx].imshow(affinity_rgb, alpha=0.5)
                axes[aff_row, vol_idx].set_title(
                    f"{aff_group['name']} Affinities Z={z} ({perc_label})", fontsize=8
                )
                axes[aff_row, vol_idx].axis("off")

            # 5. Raw slice with GT mask overlay
...
plt.suptitle(
    "3D Volumetric Training Data with Multi-Scale Affinities (1, 3, 9 voxel offsets)",
    fontsize=16,
)
plt.tight_layout()
plt.show()
...
training_results = train_3d_unet_with_memory_efficient_loader(
    ...
    num_classes=9,  # 9 affinity channels (3 offsets × 3 directions)
    ...
    affinity_offsets=[
        # Short-range: precise boundary localization
        (1, 0, 0),
        (0, 1, 0),
        (0, 0, 1),
        # Medium-range: nearby context
        (3, 0, 0),
        (0, 3, 0),
        (0, 0, 3),
        # Long-range: distant confirmation
        (9, 0, 0),
        (0, 9, 0),
        (0, 0, 9),
    ],  # Multi-scale offsets for robustness
)
...
for test_idx in test_indices:
    ...
    # Visualize multiple slices with multi-scale affinities
    fig, axes = plt.subplots(7, 5, figsize=(25, 35))  # 7 rows for multi-scale vis

    D = VOLUME_SIZE[0]
    z_indices = [0, D // 4, D // 2, (3 * D) // 4, D - 1]  # first, 25%, 50%, 75%, last

    # Define affinity offset groups
    affinity_groups = [
        {
            "offsets": [(1, 0, 0), (0, 1, 0), (0, 0, 1)],
            "channels": [0, 1, 2],
    ...
    for i, z in enumerate(z_indices):
        ...
        # Rows 1-3: Ground truth affinities at each scale (overlay on raw)
        for aff_idx, aff_group in enumerate(affinity_groups):
            row_idx = aff_idx + 1

            # Compute GT affinities for this scale
            max_offset = max([abs(o) for o in sum(aff_group["offsets"], ())])
            z_start = max(0, z - max_offset)
            z_end = min(test_gt_np.shape[0], z + max_offset + 1)
            gt_3d_patch = test_gt_np[z_start:z_end]
            ...
            # Extract slice
            slice_idx_in_patch = min(z - z_start, affinities_3d.shape[1] - 1)
            affinity_slice = affinities_3d[:, slice_idx_in_patch, :, :]  # (3, H, W)
            affinity_rgb = np.transpose(affinity_slice, (1, 2, 0))  # (H, W, 3)

            # Plot raw with GT affinity overlay
            axes[row_idx, i].imshow(test_raw_np[z], cmap="gray")
            axes[row_idx, i].imshow(affinity_rgb, alpha=0.5)
            axes[row_idx, i].set_title(f"GT {aff_group['name']} Aff Z={z}")
            axes[row_idx, i].axis("off")

        # Rows 4-6: Predicted affinities at each scale (overlay on raw)
        for aff_idx, aff_group in enumerate(affinity_groups):
            row_idx = aff_idx + 4

            # Extract predicted affinity channels for this scale
            pred_affinity_slice = prob_np[aff_group["channels"], z, :, :]  # (3, H, W)
            pred_affinity_rgb = np.transpose(
                pred_affinity_slice, (1, 2, 0)
            )  # (H, W, 3)

            # Plot raw with predicted affinity overlay
            axes[row_idx, i].imshow(test_raw_np[z], cmap="gray")
            axes[row_idx, i].imshow(pred_affinity_rgb, alpha=0.5)
            axes[row_idx, i].set_title(f"Pred {aff_group['name']} Aff Z={z}")
            axes[row_idx, i].axis("off")

    plt.suptitle(
        f"Multi-Scale Affinity Inference - Test Volume {test_idx + 1} (Acc: {volume_acc:.3f})",
        fontsize=16,
    )
    plt.tight_layout()
    plt.show()
