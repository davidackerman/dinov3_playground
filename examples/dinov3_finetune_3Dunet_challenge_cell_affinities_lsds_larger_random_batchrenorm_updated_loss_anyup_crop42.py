...
BASE_CHANNELS = 128 if DEBUG else 128  # Lower for 3D due to memory constraints
...
# %%
NUM_VOLUMES = 1 if DEBUG else 300
...
raw, gt, gt_masks, dataset_sources, num_classes = load_random_3d_training_data(
    ...
    min_unique_ids=2,
    augment=True if DEBUG else True,  # Apply data augmentations
    augment_prob=1.0,
    min_ground_truth_fraction=0.05,  # NEW: Minimum 5% GT out of total volume size
)
# print(raw.shape)

# duplicate all arrays to increase dataset size in debug mode
if DEBUG:
    raw = np.concatenate([raw] * 2, axis=0)
    gt = np.concatenate([gt] * 2, axis=0)
    gt_masks = np.concatenate([gt_masks] * 2, axis=0)
print(raw.shape)
...
val_volume_pool_size = 1 if DEBUG else 5
...
training_results = train_3d_unet_with_memory_efficient_loader(
    ...
    train_volume_pool_size=1,  # Use 15 volumes for training pool
    ...
    batches_per_epoch=1 if DEBUG else 10,  # 10 batches per epoch
