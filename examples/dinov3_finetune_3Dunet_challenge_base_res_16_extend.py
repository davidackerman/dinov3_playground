...
min_resolution_for_raw = 4  # Don't use scales finer than 2nm for raw data
...
EXPORT_BASE_DIR = (
    "/nrs/cellmap/ackermand/dinov3_training/results/challenge_base_res_16_extend"
)
...
dataset_pairs = generate_multi_organelle_dataset_pairs(
    organelle_list=["nuc", "mito", "er", "endo"],
    min_resolution_for_raw=min_resolution_for_raw,
    base_resolution=base_resolution,
    use_highest_res_for_raw=True,
...
for vol_idx in range(5):  # 5 volumes (columns)
    if vol_idx < raw.shape[0]:
        ...
        for slice_idx, (z, perc_label) in enumerate(
            ...
            # GT mask slice (show valid GT regions vs extended regions)
            axes[mask_row, vol_idx].imshow(mask_slice, cmap="RdYlBu_r", vmin=0, vmax=1)
            axes[mask_row, vol_idx].set_title(
                f"GT Mask Z={z} ({perc_label})", fontsize=8
            )
...
training_results = train_3d_unet_with_memory_efficient_loader(
    ...
    checkpoint_every_n_epochs=10,  # Save model checkpoint every 50 epochs (in addition to best)
    ...
    min_resolution_for_raw=min_resolution_for_raw,  # Raw data resolution (2nm)
    base_resolution=base_resolution,  # Ground truth resolution (8nm)
)
