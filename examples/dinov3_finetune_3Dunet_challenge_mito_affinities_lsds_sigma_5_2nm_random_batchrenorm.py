...
min_resolution_for_raw = 2  # Don't use scales finer than 32nm for raw data
...
DINOV3_SLICE_SIZE = (
    64 * base_resolution / min_resolution_for_raw
...
VOLUME_SIZE = (64, 64, 64)  # Target size for training (matches GT)
BASE_CHANNELS = 128  # Lower for 3D due to memory constraints
lsds_sigma = 5.0  # Sigma for LSD computation
...
training_results = train_3d_unet_with_memory_efficient_loader(
    ...
    train_volume_pool_size=NUM_VOLUMES - 20,  # Use 15 volumes for training pool
    val_volume_pool_size=20,  # Use 5 volumes for validation
    ...
    volumes_per_batch=10,  # Process 2 volumes per batch (memory efficient)
    ...
    loss_type="affinity_lsds",  # Use combined affinity+LSDS loss
    ...
    lsds_sigma=lsds_sigma,  # Sigma for LSD computation
    use_batchrenorm=True,  # Use BatchRenorm for more stable training
)
