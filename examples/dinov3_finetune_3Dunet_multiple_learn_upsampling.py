...
VOLUME_SIZE = (IMAGE_SIZE, IMAGE_SIZE, IMAGE_SIZE)  # Smaller for memory efficiency
...
EXPORT_BASE_DIR = (
    "/nrs/cellmap/ackermand/dinov3_training/results/multiple_3d_learn_upsampling"
)
...
num_volumes = 100  # Total number of 3D volumes to sample
...
training_results = train_3d_unet_with_memory_efficient_loader(
    raw_data=raw,
    gt_data=gt,
    train_volume_pool_size=num_volumes - 20,  # Use all but 20 volumes for training
    val_volume_pool_size=20,  # Use 20 volumes for validation
    ...
    learn_upsampling=True,  # Use learned upsampling layers
)
