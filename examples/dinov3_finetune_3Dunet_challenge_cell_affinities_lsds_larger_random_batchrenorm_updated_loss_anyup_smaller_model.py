...
if DEBUG:
    for i in range(20):
        print(
            "*************DEBUG MODE: Using smaller sizes for quick testing***********"
        )
OUTPUT_IMAGE_DIM = 128 if DEBUG else 128
DINOV3_SLICE_SIZE = (
...
VOLUME_SIZE = (
    ...
    OUTPUT_IMAGE_DIM,
    OUTPUT_IMAGE_DIM,
)  # Target size for training (matches GT)
BASE_CHANNELS = 32 if DEBUG else 128  # Lower for 3D due to memory constraints
...
# %%
NUM_VOLUMES = 4 if DEBUG else 300
...
raw, gt, gt_masks, dataset_sources, num_classes = load_random_3d_training_data(
    ...
    min_unique_ids=2,
    augment=False if DEBUG else True,  # Apply data augmentations
    augment_prob=1.0,
    min_ground_truth_fraction=0.05,  # NEW: Minimum 5% GT out of total volume size
)
...
fig, axes = plt.subplots(
    25, 10, figsize=(20, 60)
)  # 25 rows (5 types × 5 slices), 10 columns (volumes)
...
for vol_idx in range(10):  # 10 volumes (columns)
    if vol_idx < raw.shape[0]:
        ...
        for slice_idx, (z, perc_label) in enumerate(
            ...
            # Calculate row indices for the 5 image types
            raw_row = slice_idx * 5  # 0, 5, 10, 15, 20
            lsd_row = slice_idx * 5 + 1  # 1, 6, 11, 16, 21
            aff_row = slice_idx * 5 + 2  # 2, 7, 12, 17, 22
            mask_row = slice_idx * 5 + 3  # 3, 8, 13, 18, 23
            feat_row = slice_idx * 5 + 4  # 4, 9, 14, 19, 24
            ...
            # Compute LSDs (10 channels) - using safe wrapper to handle all-zero arrays
            lsds_3d = safe_get_lsds(
                gt_3d_patch, sigma=20.0
            )  # Shape: (10, small_D, H, W)
            ...
            slice_idx_in_patch = min(z - z_start, lsds_3d.shape[1] - 1)
            lsd_slice = lsds_3d[:, slice_idx_in_patch, :, :]  # Shape: (10, H, W)
            aff_slice = affinities_3d[:, slice_idx_in_patch, :, :]  # Shape: (3, H, W)

            # Display LSDs: first 3 channels as RGB
            lsd_rgb = np.transpose(lsd_slice[:3], (1, 2, 0))  # Shape: (H, W, 3)
            # Normalize to 0-1 range for display
            lsd_rgb = (lsd_rgb - lsd_rgb.min()) / (lsd_rgb.max() - lsd_rgb.min() + 1e-8)

            axes[lsd_row, vol_idx].imshow(lsd_rgb.astype(np.float32))
            axes[lsd_row, vol_idx].set_title(
                f"GT LSDs (RGB: ch 0-2) Z={z} ({perc_label})", fontsize=8
            )
            axes[lsd_row, vol_idx].axis("off")
...
plt.suptitle(
    "3D Volumetric Training Data: LSDs + Affinities (5 Percentile Slices per Volume)",
    fontsize=16,
)
plt.tight_layout()
plt.show()
...
print("=" * 60)
print("EXAMPLE 3: Memory-Efficient 3D UNet Training with Affinities + LSDs")
print("=" * 60)
...
val_volume_pool_size = 2 if DEBUG else 5
volumes_per_batch = 3 if DEBUG else 3
training_results = train_3d_unet_with_memory_efficient_loader(
    ...
    train_volume_pool_size=NUM_VOLUMES
    - val_volume_pool_size,  # Use 15 volumes for training pool
    val_volume_pool_size=val_volume_pool_size,  # Use 5 volumes for validation
    ...
    volumes_per_batch=volumes_per_batch,  # Process 4 volumes per batch (memory efficient)
    batches_per_epoch=2 if DEBUG else 10,  # 10 batches per epoch
    ...
    # LOSS FUNCTION PARAMETERS - Use affinity+LSDS loss
    loss_type="boundary_affinity_focal_lsds",  # Use combined affinity+LSDS loss
    # AFFINITY+LSDS PARAMETERS
    output_type="affinities_lsds",  # Enable affinity+LSDS output
    affinity_offsets=affinity_offsets,  # Affinity offset directions
    lsds_sigma=20.0,  # Sigma for LSD computation
    mask_clip_distance=3,
    use_batchrenorm=True,  # Use Batch Renormalization for stability
    use_anyup=True,  # Use AnyUp for upsampling
)
