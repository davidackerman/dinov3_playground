...
base_resolution = 64  # Resolution for segmentation labels
min_resolution_for_raw = 16  # Don't use scales finer than 32nm for raw data
...
for vol_idx in range(10):  # 5 volumes (columns)
    if vol_idx < raw.shape[0]:
        ...
        for slice_idx, (z, perc_label) in enumerate(
            ...
            gt_row = slice_idx * 4 + 1  # 1, 5, 9, 13, 17
            ...
            axes[gt_row, vol_idx].imshow(affinity_rgb)
            axes[gt_row, vol_idx].set_title(
                f"GT Affinities (RGB) Z={z} ({perc_label})", fontsize=8
            )
...
plt.suptitle(
    "3D Volumetric Training Data with GT Extension Masks (5 Percentile Slices per Volume)",
    fontsize=16,
)
plt.tight_layout()
plt.show()
